...
function UnLuaPerformanceTestProxy:ReceiveBeginPlay()
	...
	local PredictedPositions = TArray(FVector)
	StartTime = Seconds()
	for i=1, N do
	...
	local Origin = FVector(1.0, 1.0, 1.0)
	local Direction = FVector(1.0, 0.0, 0.0)
	StartTime = Seconds()
	for i=1, N do
